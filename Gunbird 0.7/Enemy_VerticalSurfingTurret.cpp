...
Enemy_VerticalSurfingTurret::Enemy_VerticalSurfingTurret(int x, int y) : Enemy(x, y)
{
	...
	water.PushBack({ 239, 509,32,38 });
	water.PushBack({ 239, 550,32,38 });
	water.PushBack({ 239, 587,32,38 });
	water.PushBack({ 239, 625,32,38 });
	water.PushBack({ 239, 661,32,38 });
	water.PushBack({ 239, 697,32,38 });
	...
	collider = App->collision->AddCollider({ 100, 0, 16, 24 }, COLLIDER_TYPE::COLLIDER_ENEMY, (Module*)App->enemies);
...
void Enemy_VerticalSurfingTurret::Shoot() {
	if (App->sea->pause == false) {
