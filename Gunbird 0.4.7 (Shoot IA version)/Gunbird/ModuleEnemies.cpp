	...
	for (uint i = 0; i < MAX_ENEMIES; ++i)
	{
		if (enemies[i] != nullptr && enemies[i]->GetCollider() == c1)
		{
			if (c1->type == COLLIDER_POWER_UP) {
				...
				if (big_enemy_life == 0) {
					App->particles->AddParticle(App->particles->explosion, enemies[i]->position.x - 25, enemies[i]->position.y - 25,0,0.1);
				...
				if (big_enemy_life > 0) {
					App->particles->AddParticle(App->particles->damage_balloon, enemies[i]->position.x, enemies[i]->position.y,0,0.1);
					big_enemy_life--;
				}
				break;
			}
			else {
				App->particles->AddParticle(App->particles->littleexplosion, enemies[i]->position.x - 10, enemies[i]->position.y - 5,0,0.1);
				App->audio->LoadFX("Audio/explosion.wav");
